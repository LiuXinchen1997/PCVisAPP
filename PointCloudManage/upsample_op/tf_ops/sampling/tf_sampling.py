...
if __name__=='__main__':
    ...
    input_dir = '/data1/liuxinchen/temp'
    output_dir = './'
    ...
    for file in files:
        ...
        num_sample_points = int(points.shape[0] / 4)
        points = np.expand_dims(points, axis=0)
        
        with tf.Session() as sess:
            idx = farthest_point_sample(num_sample_points, points)
            sampleData = gather_point(points, idx)
            _, sample_points = sess.run([idx, sampleData])
        
        print(sample_points.shape)
        np.savetxt(os.path.join(output_dir, os.path.basename(file)), sample_points[0])
